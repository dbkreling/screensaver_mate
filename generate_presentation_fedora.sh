...
<!-- This animation will start at midnight. -->
  <static>
    ...
    <file>$PWD/$PIXDIR/$pic1</file>
  </static>
  <transition>
    <duration>5.0</duration>
    <from>$PWD/$PIXDIR/$pic1</from>
EOF

for (( c=1; c<=${#pictures[@]}-1; c++ )); do
cat <<EOF >> $PIXDIR/background-2.xml
    <to>$PWD/$PIXDIR/${pictures[$c]}</to>
  </transition>
  <static>
    <duration>1795.0</duration>
    <file>$PWD/$PIXDIR/${pictures[$c]}</file>
  </static>
  <transition>
    <duration>5.0</duration>
    <from>$PWD/$PIXDIR/${pictures[$c]}</from>
EOF
done

cat <<EOF >> $PIXDIR/background-2.xml
    <to>$PWD/$PIXDIR/$pic1</to>
  </transition>
</background>
EOF
